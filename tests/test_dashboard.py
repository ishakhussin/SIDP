...
class DashboardTest(unittest.TestCase):
    ...
    def test_dashboard_script_uses_v123_camera_api(self):
        ...
        try:
            ...
            self.assertIn(b"AI monitoring is inactive", response.data)
            self.assertIn(b"bg-green-500/20 text-green-400", response.data)
            self.assertIn(b"capture_fps", response.data)
            self.assertIn(b"/restricted-zone?preset=${preset}", response.data)
            self.assertIn(b"/patrol", response.data)
            self.assertIn(b"Auto Patrol active", response.data)
